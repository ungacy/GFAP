...
def argparseFunc():
  ...
  parser.add_argument("-pvalue_index",type=int,help="the column index of the p-value")
...
def align_annotation(filepath,inputtype,annotype,refspecies,evalue,algnment_type,algnment_percent,save,cpu,only_ID=None):#设定保存的路径，保存的一些参数需要变动
    ...
    if inputtype=="protein":
        ...
        if algnment_type!="sensitive":
            ...
            result=subprocess.Popen(args,shell=True)
            result.wait()
    if os.path.getsize(save+"/GFAP-blastresult.txt")!=0:
        if "GO" in annotype:
        ...
        message="Sorry, the homologous genes of the input gene were not detected in the database under the evalue of 1e-5.\nTherefore, you may gain annotation results by changing the evalue (such as setting evalue=0.1).\n or you can utilize the complete database for annotation instead of sequence alignment. If you still have some questions, you can directly contact me at xudongzhuanyong@163.com"
        with open(save+"/GFAP-"+refspecies+"_annotation_errors_and_the_solution.txt","w") as f:
            f.write(message)
...
def plantannotate(filepath,evalue,savefile,seqtype,annotatype,databasetype,cpu,only_ID=None):
    ...
    if seqtype=="CDS" or seqtype=="protein":
        ...
        if contents==[]:
            judgement="The domains contained in the input file seemed not to be annotated by GO/KEGG/Pfam. or you can change the evalue and try again!"
            with open(savefile+"/annotation_result.txt","w") as f:f.write(judgement)
...
def nr_swissprot(filepath,inputtype,databasetype,evalue,algnment_type,cpu,algnment_percent,save):#设定保存的路径，保存的一些参数需要变动
    ...
    if os.path.getsize(save+"/GFAP-blastresult.txt")!=0:
        ...
        message="Sorry, the homologous genes of the input gene were not detected in the database under the evalue of 1e-5.\nTherefore, you may gain annotation results by changing the evalue (such as setting evalue=0.1).\nIf you still have some questions, you can directly contact me at xudongzhuanyong@163.com"
...
def singlefamily(filepath,model,save,cpu,evalue,model2=""):#model2允许用户自己加入
    ...
    if contents!=[]:
        ...
        judgement="The domains contained in the input file seemed not to be annotated. or you can change the evalue and try again!"
        with open(savefile+"/annotation_result.txt","w") as f:f.write(judgement)
    ...
    if contents!=[]:exseqs(filepath,save,lines)
...
def mergef(file,save,pathway):
    ...
    for file in filelist:#强制性要求用户名字中必须含有关键词
        ...
        if os.path.getsize(file)!=0:
            if "GO" in file and "ID" not in file:
                gof=funccollect(file)
            if "kegg" in file and "ID" not in file:
...
def drawstatistics(file,cut_value,drawtype,color,colormodel,gene_number,save,singlecolor=False,go=False,kegg=False,multi_families=False):
    ...
    if go:
        ...
        if drawtype=="bar_chart":
            drawkp(filepath,cutvalue,color,gene_number,save,singlecolor)
        if drawtype=="heatmap":
            heatmap_kegg(file,cutvalue,colormodel)
        if drawtype=="select draw type   ":
            drawkp(filepath,cutvalue,color,gene_number,singlecolor,save)
    elif multi_families:
        if drawtype=="bar_chart":
            drawkp(filepath,cutvalue,color,gene_number,singlecolor,save)
        if drawtype=="heatmap":
...
def predict_long_transcript(file,save,length="100"):
    ...
    for f in director:
        if ".cds" in f :
            argue2="cp ./"+filename+".transdecoder_dir/"+f+" "+savedir+"/"+f
...
if __name__ == '__main__':
    ...
    if (args.query_cds!=None or args.query_protein!=None) and (args.annotation_with_species!=None or args.annotation_with_database!=None):
        ...
        if ">" in file:#如果输入的仅仅只是序列，则在后台给它建个文件，所以老弟，前端一开始的那个输入框需要动一下，那个输入框接收的仅仅只是一个基因的序列
            with open("./target_seq/target_seq.txt","w") as f:
                f.write(file)
            file="./target_seq/target_seq.txt"
    ...
    if args.draw_statistics:
        ...
        if drawtype!="heatmap" or drawtype!="bar_chart":drawtype="select draw type   "
        if color==None:color=""
    ...
    if args.draw_network:
        ...
        allrevigo(speciesname,number_threshold,threshold,filename,func,numberthreshold,rmodel,save,network=True,go=go,kegg=kegg,pfam=multi_families)
    if args.predict_long_transcript:
        seqs=args.query_cds
        if ">" in seqs and seqs.count(">")>=10:
            with open("./transcript_seq.txt","w") as f:f.write(seqs)
            file="./transcript_seq.txt"
        else:
            file=seqs
        predict_long_transcript(file,save)
